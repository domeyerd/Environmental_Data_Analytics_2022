...
NTL.nutrient.data.PeterPaul <- droplevels(NTL.nutrient.data.PeterPaul) #this drops the zero values.
summary(NTL.nutrient.data.PeterPaul$lakename)
...
dim(Trap)
dim(LitterTrap)
# Note select only works on columns, not rows.
...
```{r}
# The filter part takes N/As out of only those columns. The "&" means that you are filtering out rows where all three columns have N/As.
