...
```{r}
high <- c(49, 36, 26, 45, 48, 55, 55, 54, 45, 39)
low <- c(31, 8, 9, 25, 29, 39, 48, 42, 21, 19)
...
celciusH <- convert(high)
celciusL <- convert(low)
...
temp <- data.frame(high, celciusH, low, celciusL)
colnames(temp) <- c("High.F", "High.C", "Low.F", "Low.C")
...
format(today, format = "%y") #this returns just the last two digits
format(today, format = "%m-%d-%Y") #capital Y returns the full year
class(today) #this is character
```
