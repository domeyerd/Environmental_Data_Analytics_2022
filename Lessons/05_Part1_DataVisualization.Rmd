...
#Assigning the graph then allows you to control where it appears by printing, also helpful for putting multiple graphs next to each other.
O3plot <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone))
print(O3plot)

# Fix this code. color has to be outside the "AES" code, otherwise its like adding another layer to the graph. 
O3plot2 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone), color = "blue")
print(O3plot2)

# Add additional variables
# the shape segment with "as.factor(Year)" assigns different shapes based on the year factor. Color is inside the aes command now because its another layer. The color is by Site Name.
PMplot <- 
  ggplot(EPAair, aes(x = Month, y = PM2.5, shape = as.factor(Year), color = Site.Name)) +
  geom_point()
print(PMplot)

# Separate plot with facets. This gives what you've wrapped its own plot 
PMplot.faceted <-
...
# Filter dataset within plot building and facet by multiple variables. Only plots some variables. "facet_grid(Site.Name ~ Year)" - rows are Site.Name and Year is columns.
PMplot.faceted2 <- 
...
# Plot true time series with geom_line. Typical example of a time-series plot.
PMplot.line <- 
...
# Depth in the fields of limnology and oceanography is on a reverse scale. If you dont specify scale_y_reverse, then the 0 won't be on top instead of bottom (for depth data in this case)
tempvsdepth <- 
