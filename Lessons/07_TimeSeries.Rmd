...
```{r}
#Starts in January, 1985. Increases by 1 day for every new observation based on frequency. 
...
Must be univariate. Never give more than one variable. 
...
#Adding the observed and date columns. 
...
# In real applications you will simply replace NAs, on the original column

# In this example, the package "zoo" is specified for "na.approx" because there are multiple functions with that same name. You also don't need to library the package when you do this. 
wind_data_clean <- 
  wind_data %>% 
  mutate( AWND.clean = zoo::na.approx(AWND) )

summary(wind_data_clean$AWND.clean)
#Note the NA is gone, but the main properties of the data set are the same. Interpolation will never change the mean, median, min, max, etc. of the dataset. 

#This is just to visualize the na.approx. Showing that the line is connected.If the data point is in the middle of the data set, never use na.omit always replace it with na.approx.
...
* **Mann-Kendall**: no seasonality, non-parametric, missing data allowed. Function: `MannKendall()` (package: Kendall). Can still be called monotonic.
* **Seasonal Mann-Kendall**: seasonality, non-parametric `SeasonalMannKendall` (package: Kendall). Can still be called monotonic.
* **Spearman Rho**: no seasonality, non-parametric, missing data allowed. Function: `cor.test(method="spearman")` (package: stats). Cannot handle seasonal data.
...
#Define first month and first year. 
f_month <- month(first(wind_data_clean$DATE))
f_year <- year(first(wind_data_clean$DATE))
#Frequency is 12 becuase working with monthly data.
...
# Run SMK test. Only test that can handle seasonal data.
wind_data_trend1 <- Kendall::SeasonalMannKendall(wind_data_ts)

# Inspect results
wind_data_trend1
summary(wind_data_trend1)
# p-value is less than 0.05. This means that the data is not stationary and we have identified a trend. 

# This is the other method for seasonal. This gives trend value for each season. So for season 4 and 5 the trend is actually stationary. 
