...
# You can only compare data that's the same class. 

# matrix filtering
NTL.phys.data.surface1 <- NTL.phys.data[NTL.phys.data$depth == 0,]

# dplyr filtering
NTL.phys.data.surface2 <- filter(NTL.phys.data, depth == 0) # This does same as above
...
NTL.phys.data.JunethruOctober1 <- filter(NTL.phys.data, daynum > 151 & daynum < 305) # 151 is an arbitrary number in this dataset that begins June, same with 305 for October
...
# Creates a dataframe with a subset of the data, only lakename and the columns between sampledate and temp. lakename isn't adjacent so you have to call it out individually.
...
#This does the same thing to find first and last date.
...
```{r}
# Because you're running it as a pipe, you don't need to include the dataframe name every time. 
...
Saving processed datasets in their own folder, and not overwriting the raw data is good practice. 
